...
A PDF is two cases, not one, and treating it as one was a mistake this file
made for a while. Almost every CFDI PDF is generated by billing software and
printed straight to PDF, so the characters are already in the file — the text
layer is *there*, positioned but never rasterized. Measured on 93 real
PDF/XML pairs, that text holds 99% of the checked fields; the best local
vision model reading the same pages as images holds 83%. So a PDF with a text
layer is read, not looked at, and only a scan or a photo reaches the vision
path.
...
def route_document(
    ...
    *,
    ...
    #
    # A PDF that carries its own text is still tier 1: a model maps text to
    # fields, which is a real task. What it is not is a transcription task, and
    # that is the difference the measurement shows.
    if media == PDF and has_text_layer(data):
        if provider is None and not get_config().llm_enabled:
            raise UnroutableDocument(
                "el PDF tiene capa de texto, pero mapear texto a campos "
                "requiere un modelo y no hay credenciales configuradas "
                "(ANTHROPIC_API_KEY, o LLM_PROVIDER=local con LLM_BASE_URL)"
            )
        from cfdi_agent.extract.text_extract import extract_from_text_layer

        try:
            invoice, result = extract_from_text_layer(data, provider=provider)
        except ProviderError as exc:
            raise UnroutableDocument(f"extracción de capa de texto falló: {exc}") from exc
        return RoutedDocument(
            invoice=invoice,
            media_type=media,
