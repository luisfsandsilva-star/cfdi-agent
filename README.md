...
**Result on 95 real invoices from 56 different issuers.** All 95 parse. All 95
validate against the SAT schema chain. No critical anomaly fires that survives
inspection.

Getting there needed three fixes, and all three were this software being wrong
about correct invoices. A stamped CFDI is a strong prior for that: the issuer's
PAC validates the arithmetic before it stamps, so a critical arithmetic firing
is first evidence against the parser.

*Local taxes were not counted.* Two invoices carry an `implocal` complement —
state-level taxes. They are absent from `cfdi:Impuestos` because they are not
federal, but the issuer charges them, so the expected total was short by
exactly `TotaldeTraslados`: 254.51 and 129.51, matching the reported
differences to the cent.

*The subtotal tolerance did not scale with line count.* Each line amount is
rounded to cents by the issuer, so n lines can drift up to half a cent each
from a subtotal computed before rounding. A 4-line invoice missed by 0.02.

*The schema chain was incomplete.* Four documents were reported as invalid, and
they are valid: `cfdi:Complemento` is an `xs:any processContents="strict"`, so
a complement schema is never reached by following imports. `implocal` and
`aerolineas` are now vendored, and conformance is 95/95.

The corpus also contains complementos the generator produces none of, one
currency and one document type. Two invoices in 95 have no serie or folio at
all, which the generator always writes.

`semantic_duplicate` fires 3 times, and those are reported as measured rather
than tuned away. Each is the same issuer, an identical description, an
identical round total — 400, 480 and 600 — days apart. Small recurring
purchases look exactly like double billing to a rule built on issuer, amount
and a 7-day window. Whether they should alert is a question for the business,
not a threshold to fit to three samples with no ground truth.
...
The first thing that measures is whether a model is needed at all. A CFDI PDF
is generated by billing software and printed straight to PDF, so the characters
are already in the file — positioned, never rasterized.

93 PDF/XML pairs, fields present in the extracted text:

| field | `PaddleOCR-VL-1.6` | **PDF text layer** |
|---|---:|---:|
| `uuid` | 72% | **99%** |
| `rfc_emisor` | 94% | **100%** |
| `rfc_receptor` | 96% | **100%** |
...
| latency per invoice | 10 s | **<0.1 s** |

Reading the text layer costs nothing and cannot invent a digit, because there
is no transcription step to go wrong. Nobody checked that before two vision
models were downloaded and measured. It is the same mistake this project's
thesis is about — reaching for a model on data that is already structured —
made one level down, inside the path that exists for when the structure is
missing.

The vision path stays, because the two fail on different documents. One invoice
renders its stamp block as an image while keeping the labels as text, so the
text layer reads "Folio Fiscal" and finds nothing after it.
`cfdi_agent.extract.pdf_text.has_text_layer` is the switch, and a scan trips it
honestly by yielding almost no characters.

Full model comparison, including the two that were ruled out and why:
